...
fn run_tui(max_retries: usize, command_timeout: u64) -> Result<()> {
    ...
    while app.running {
        ...
        // (AwaitingUserChoice event will transition us to completion screen)
        if app.screen == Screen::LoopExecution {
            app.process_loop_events();
        }

        // Process loop events on completion screen to receive Complete event
        // after orchestrator finishes cleanup
        if app.screen == Screen::LoopCompletion && app.completion_data.in_progress {
            let completed = app.process_loop_events();
            if completed {
                // Cleanup finished - transition to result screen
                app.cleanup_loop();
